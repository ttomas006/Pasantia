<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Pepsi</title>
    <link rel="stylesheet" href="css/pepsi-wildcherry-and-cream.css">
</head>
<body>
    <main>
        <header>
            <li class="Enlace__logo">
                        <a href="index.html"><img src="https://www.pepsi.com/assets/images/pepsi-logos/logo-0.png" alt="Logo de Pepsi"></a>
            </li>
            <button class="menu-btn">≡</button>
            <nav>
                <ul class="menu-items">
                        <button class="menu-btn-2">✕</button>
                        <img src="https://www.pepsi.com/assets/images/pepsi-logos/logo-0.png" alt="">
                         <li class="items_inicio"><a href="index.php">Inicio</a></li>
                        <li class="items_productos"><a href="pepsi.php">Productos</a></li>
                        <li class="items_contacto"><a href="contacto.php">Contacto</a></li>
                        <li class="items_entrevista"><a href="entrevista.php">Entrevista</a></li>
                </ul>
            </nav>
        </header>
        <div class="contenedor">
        <div class="hoja">
            <div class="info_nutri">
                <h1 class="nombre-bebida">PEPSI WILDCHERRY AND CREAM</h1>
                <hr class="linea">
                <h2>Informacion Nutricional</h2>
                <hr class="linea">
                <p>Tamaño de la porción:12fl oz Porciones por envase: 1</p>
                <p>Cantidad por porción
                <hr>
               <div><p class="derecha">% Valor diario</p></div> 
                <div>
                    <p>Calorías: 150</p>
                </div>
                <div class="kiko">
                    <p>Grasa total: 0 g</p>
                    <p>0 %</p>
                </div>
                <div class="kiko">
                <p>Sodio: 30mg</p> 
                <p>1 %</p>   
                </div>
                <div class="kiko">
                 <p>Carbohidratos totales: 41g</p>   
                 <p>15 %</p>  
                </div>
                <p>Azúcares: 41g</p>    
                <p id="proteina">Proteína: 0g</p>
                <hr>
                <p class="ingredientes">AGUA CARBONATADA , JARABE DE MAÍZ DE ALTA FRUCTOSA , SABOR NATURAL Y ARTIFICIAL , COLOR CARAMELO , ÁCIDO FOSFÓRICO , CAFEÍNA , ÁCIDO CÍTRICO</p>
            </div>
        </div>
        </div>
             <div class="flechas">
                <a class="flecha-derecha" href="pepsi-real-sugar.php">←</a>
                <a class="flecha-izquierda" href="pepsi-zero-sugar-wildcherry-and-cream.php">→</a>
            </div>
    </main>
<script src="js/productos.js"></script>
<script src="js/index.js"></script>
</body>
</html>